...
NSURL *server;
NSString *site;
NSTimeInterval const cookieDuration = 2629743;
...
- (void)loadCookiesFromStorageForServer: (nonnull NSURL *) serverUrl {
    ...
    NSString *accessTokenString = [self getTokenFromKeychain:accessToken];
    if (accessTokenString != nil) {
        [self addCookie: accessTokenString forIdentifier:cookieNameString(accessCookie) forDuration:cookieDuration];
    }
    
    NSString *xsrfTokenString = [self getTokenFromKeychain:xsrfToken];
    if (xsrfTokenString != nil) {
        [self addCookie: xsrfTokenString forIdentifier:cookieNameString(xsrfCookie) forDuration:cookieDuration];
    }
}
...
/**
 ...
 This isn't fully baked yet: if the webview loads a page that blanks out the
 @param serverUrl
 The name of the server.
 */
- (void)requestTokensForSession: (nonnull NSURL *) serverUrl {
    
    // Get the wg_session_id cookie from cookie storage.
    NSHTTPCookie *cookie;
    ...
    if (sessionCookie != nil) {
        ...
        if ((previousSessionID == nil) || (![previousSessionID isEqualToString:sessionCookieValue])) {
            // We haven't seen this session id before, so after checking our sign in status, request oauth tokens.
            
            previousSessionID = sessionCookieValue;
            [self checkSignInStatus:serverUrl forSite:@"" successCallback:^{
                // Signed in.
                [self requestOAuthTokens: serverUrl];
            } failureCallback:^{
                // Not signed in, our session id is stale.
            }];
        }
    }
}
...
- (void)requestOAuthTokens: (nonnull NSURL *) serverUrl {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@/oauth2/v1/token", server.scheme, server.host]];
...
- (void) revokeToken: (tokenName) tokenName {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@/oauth2/v1/revoke", server.scheme, server.host]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:60.0];
    [request setHTTPMethod:@"POST"];
...
- (void)checkSignInStatus: (nonnull NSURL *) serverUrl forSite:(nonnull NSString *) siteName successCallback:(nullable void (^)(void))successCallbackBlock failureCallback:(nullable void (^)(void))failureCallbackBlock {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@/vizportal/api/web/v1/getSessionInfo", server.scheme, server.host]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:60.0];
    [request setHTTPMethod:@"POST"];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if(connection == tokenConnection) {
        [self parseTokensJSON:(NSData *)_tokenResponseData];
    } else if(connection == signInStatusConnection) {
    ...
    if ([keys containsObject:@"error"]) {
        ...
        for (id key in keys) {
            ...
            if ([keyAsString isEqual: tokenNameString(accessToken)]) {
                ...
                [self addCookie:[self getTokenFromKeychain:accessToken] forIdentifier:cookieNameString(accessCookie) forDuration:cookieDuration];
                ...
                [self addCookie:[self getTokenFromKeychain:xsrfToken] forIdentifier:cookieNameString(xsrfCookie) forDuration:cookieDuration];
            }
        }
